...
local add_install_command = function()
  ...
  vim.api.nvim_exec(
    [[command! -nargs=0 -bar -buffer InstallJsonLint lua require("my.install.jsonlint").install()]],
    false
  )
end
...
local remove_install_command = function()
  -- TODO: When v0.7 is released: https://github.com/nanotee/nvim-lua-guide#defining-user-commands
  vim.api.nvim_exec([[delcommand InstallJsonLint]], false)
end

M.add_install_command_if_not_detected = function()
  if not require("my.install.jsonlint").detect() then
    require("my.install.jsonlint").add_install_command()
  end
end
