...
local add_install_command = function()
  ...
  vim.api.nvim_exec(
    [[command! -nargs=0 -bar -buffer InstallYamllint lua require("my.install.yamllint").install()]],
    false
  )
end
...
local remove_install_command = function()
  -- TODO: When v0.7 is released: https://github.com/nanotee/nvim-lua-guide#defining-user-commands
  vim.api.nvim_exec([[delcommand InstallYamllint]], false)
end

M.add_install_command_if_not_detected = function()
  if not require("my.install.yamllint").detect() then
    require("my.install.yamllint").add_install_command()
  end
end
...
M.install = function()
  ...
  if vim.fn.executable("pip3") ~= 1 then
    notify.error("pip3 not found which is required. Please install Python 3 and retry.")
    return false
  end

  local job = Job:new({ command = "pip3", args = { "install", "--user", program } })
  job:sync()
  if job.code == 0 then
